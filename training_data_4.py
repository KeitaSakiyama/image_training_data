# -*- coding: utf-8 -*-
"""teaching_data_4.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1fkwCsMoBcSPE21h5E3LXRTJHk5Bkq8j5
"""
    ...
    for _ in range(num_overlays):
        while True:
            ...
            for (placed_x, placed_y, placed_w, placed_h) in placed_positions:
                distance = math.sqrt((x_center - placed_x)**2 + (y_center - placed_y)**2)

                #一つでも不適切ならFalse、そうでなければTrueのまま
                #中心間距離distanceがすでに配置済みの物体の半径より小さい or これから配置する物体の半径より小さい　or　物体の半径の3倍より大きいときfalse
                if (distance < math.sqrt((placed_w)**2 + (placed_h)**2)/2
                  or distance < np.linalg.norm(new_size)/2
                  or distance > min(math.sqrt((placed_w)**2 + (placed_h)**2)/2, np.linalg.norm(new_size)/2)*3):
...
# 出力ディレクトリを設定　→　generated_images4
output_directory = 'image_detection/generated_images4'
